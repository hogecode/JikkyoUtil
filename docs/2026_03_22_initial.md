...
CLIアプリを作りたい。
実装したいのは以下のコマンドです。
jikkyo --title(-t) {string} --episode(-e) {number} --verbose(-v)
...
1. jikkyo --title(-t) {string} --episode(-e) {number} --verbose(-v) というコマンドをユーザーが入力する。

例えば、jikkyo -t ぷり -e 10 -vと入力した場合、以下のURLのSearchの位置に-tで入力した文字を代入する。
